...
// ==========================================================================
// 12. CONFIGURAÇÕES POR PROJETO E HERANÇA DE DEFAULTS GERAIS (Issue #17)
// ==========================================================================
...
export async function saveProjectSettings(projectId) {
  ...
  // Coleta apenas campos marcados nos overrides
  const overrides = {};
...
export function initProjectSettingsEvents() {
  // 1. Intercepta o clique nos projetos da sidebar de configurações para manter o modal aberto e chamar openProjectSettings
  ...
  if (projectsNavList) {
    projectsNavList.addEventListener('click', (e) => {
      ...
      // Interrompe imediatamente a propagação para evitar fechamento do modal por handlers legados
    ...
    }, true); // useCapture: true para interceptar antes de qualquer outro handler
  }

  // 2. Eventos nos botões de alternância de herança (toggle override)
  ...
  // 3. Botão de salvar overrides
  const btnSave = document.getElementById('btn-save-project-settings');
  if (btnSave) {
    btnSave.addEventListener('click', () => saveProjectSettings());
  }

  // 4. Botão de restaurar padrões gerais
  const btnRestore = document.getElementById('btn-restore-project-defaults');
  if (btnRestore) {
...
// Auto-inicializa governança, temas, customizações e configurações de projetos
